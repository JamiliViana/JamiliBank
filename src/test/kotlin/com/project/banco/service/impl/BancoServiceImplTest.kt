...
class BancoServiceImplTest {
    ...
    fun deveLancarValueNotAcceptedException_QuandoChamarDeposito() {
        val cpf = "123.456.789-10"
        val valor = -10.0
        val conta = Conta(cpf = cpf, saldo = 100.0)
    ...
    fun deveLancarContaDestinoNotFoundException_QuandoChamarTransferencia(){
        ...
        every { bancoPersistenceAdapter.findByCpf(cpfOrigem) } returns contaOrigem
        every { bancoPersistenceAdapter.findByCpf(cpfDestino) } returns null
    ...
    fun deveRetornarSameAccountException_QuandoChamarTransferencia(){
        ...
        every { bancoPersistenceAdapter.findByCpf(cpfOrigem) } returns contaOrigem
        every { bancoPersistenceAdapter.findByCpf(cpfDestino) } returns contaOrigem
